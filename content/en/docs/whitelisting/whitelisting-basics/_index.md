...
## Begin Allowlisting Programs After Installation

While running, HeartSuite always attempts to capture permission errors to the HeartSuite log, `/.hs/sys/hs-activity-log.txt`. Initially, it will report programs that are executed and that are not allowlisted. allowlist entry. For example, if you run the nano program before allowlisting it, the HeartSuite log will contain the following entry:
...
Otherwise, you can extract the allowlist entry number using the list command, coupled with a simple grep:
...
You can add this IP address to the allowlist entry for wget as follows:

```bash
# sudo /.hs/sys/hs-allowlist-manager add -x /usr/bin/wget -n 45.60.22.168
```

– or –

```bash
# /.hs/sys/hs-allowlist-manager list | grep wget
# /.hs/sys/hs-allowlist-manager add -r 277 -n 192.142.166.196
# /.hs/sys/hs-allowlist-manager list | grep wget
277
/usr/bin/wget
# /.hs/sys/hs-allowlist-manager add -r 277 -n 192.142.166.196
...
Use the dashboard review tools as the primary method, or add manually with hs-allowlist-manager (see --help for examples).
Use the dashboard review tools as the primary method.

Please note that the review tools provide metadata and safe grouping to avoid raw logs or blind bulk approvals. We recommend beginning with the program, file, and network review tools from the dashboard. It can take several days to a week to capture events from systemd timers and cron jobs. Once captured, use the dashboard tools to tailor permissions on a need-to-access basis. Grouped reviews can handle similar permissions safely with samples.