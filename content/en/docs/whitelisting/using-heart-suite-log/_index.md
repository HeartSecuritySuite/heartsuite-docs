...
categories: ["Guides"]
tags: ["heartsuite", "linux", "permissions", "whitelist", "log", "security"]
type: docs
toc: true
menu:
  main:
...
While running, HeartSuite always attempts to capture permission errors to the HeartSuite log, `/.hs/sys/hs-activity-log.txt`. Initially, it will report programs that are executed and that are not whitelisted. For example, if you run the nano program before whitelisting it, the HeartSuite log will contain the following entry:
...
```bash
# /.hs/sys/hs-whitelist-manager list | grep nano
```
