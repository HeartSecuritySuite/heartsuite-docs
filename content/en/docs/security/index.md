...
|-----|-----------|-----------|-----------------|--------|
| [CVE-2026-31431](#cve-2026-31431) | algif_aead (AF_ALG) | <span class="badge bg-secondary">—</span> | <span class="badge badge-erased">0.0</span> | Code not compiled in |
...
| [CVE-2024-47682](#cve-2024-47682) | SCSI subsystem (`CONFIG_SCSI`) | <span class="badge badge-cve-high">7.8 HIGH</span> | <span class="badge badge-cve-high">7.3 HIGH</span> | Affected — `CONFIG_SCSI=y`; Lockdown limits post-exploitation |
...
| [CVE-2024-56631](#cve-2024-56631) | SCSI generic driver (`CONFIG_CHR_DEV_SG`) | <span class="badge badge-cve-high">7.8 HIGH</span> | <span class="badge badge-erased">0.0</span> | Risk erased — `/dev/sg*` not in allowlist; Lockdown prevents modification |
...
| [CVE-2023-53285](#cve-2023-53285) | ext4 filesystem (`CONFIG_EXT4_FS`) | <span class="badge badge-cve-high">7.8 HIGH</span> | <span class="badge badge-cve-high">7.3 HIGH</span> | Affected — `CONFIG_EXT4_FS=y`; Lockdown limits post-exploitation |
...
| [CVE-2023-3268](#cve-2023-3268) | relay filesystem (`CONFIG_RELAY`) | <span class="badge badge-cve-high">7.1 HIGH</span> | <span class="badge badge-erased">0.0</span> | Risk erased — debugfs relay not in allowlist; Lockdown prevents modification |
...
| [CVE-2024-49860](#cve-2024-49860) | ACPI subsystem (`CONFIG_ACPI`) | <span class="badge badge-cve-high">7.1 HIGH</span> | <span class="badge badge-cve-high">7.1 HIGH</span> | Affected — `CONFIG_ACPI=y`; base I:N, Lockdown limits post-exploitation persistence |
...
| [CVE-2024-56606](#cve-2024-56606) | AF_PACKET sockets (`CONFIG_PACKET`) | <span class="badge badge-cve-high">7.8 HIGH</span> | <span class="badge badge-erased">0.0</span> | Risk erased — `CAP_NET_RAW` not in allowlist; Lockdown prevents modification |
...
| [CVE-2023-53111](#cve-2023-53111) | loop block device (`CONFIG_BLK_DEV_LOOP`) | <span class="badge badge-cve-high">7.8 HIGH</span> | <span class="badge badge-erased">0.0</span> | Risk erased — `/dev/loop*` not in allowlist; Lockdown prevents modification |
...
| [CVE-2026-23378](#cve-2026-23378) | network traffic scheduler (`CONFIG_NET_SCHED`) | <span class="badge badge-cve-high">7.8 HIGH</span> | <span class="badge badge-erased">0.0</span> | Risk erased — `tc` not in allowlist; Lockdown prevents modification |
...
| [CVE-2024-50112](#cve-2024-50112) | x86_64 architecture (`CONFIG_X86_64`) | <span class="badge badge-cve-high">7.8 HIGH</span> | <span class="badge badge-erased">0.0</span> | Risk erased — no LAM-using process in HS allowlist |
...
**Base Score**: 7.8 HIGH (AV:L/AC:L/PR:L/UI:N/S:U/C:H/I:H/A:H)  
**Environmental Score**: 7.1–7.3 HIGH — Lockdown reduces MI: High→Low (no allowlist modification, no persistence, no backdoors); C and A remain High; score stays within the HIGH band  
...
HeartSuite Lockdown blocks the post-exploitation path: `sys_hs_lockdown_hs()` sets `HS_lockdown_state = 7`, after which `FS_IOC_SETFLAGS` returns `EPERM` (`kernel/ioctl.c:561–569`) and all mount paths return `EPERM` (`kernel/namespace.c:4218, 4300, 4453`). An attacker who reaches root cannot modify the allowlist, install persistent backdoors, or survive a reboot.

HeartSuite Secure Mode enforces an SPF allowlist in `hs_sandbox_caching.c` that applies equally to root. Reaching the vulnerable io_uring path requires an approved process to submit a crafted SQE. An attacker cannot execute a new exploit binary — it has no allowlist entry and the kernel refuses to run it. Together, Lockdown and Secure Mode place this vulnerability in a contained environment where exploitation leads to no durable compromise.
...
HeartSuite Secure Mode enforces an SPF allowlist in `hs_sandbox_caching.c` that applies equally to root. Reaching the xattr teardown path requires an approved process to manipulate extended attributes on an ext4 filesystem. An attacker cannot execute a new exploit binary — it has no allowlist entry and the kernel refuses to run it. Together, Lockdown and Secure Mode place this vulnerability in a contained environment where exploitation leads to no durable compromise.
...
HeartSuite Secure Mode enforces an SPF allowlist in `hs_sandbox_caching.c` that applies equally to root. Triggering this path requires an approved process to mount a filesystem image containing a crafted directory. An attacker cannot execute a new exploit binary — it has no allowlist entry and the kernel refuses to run it. Together, Lockdown and Secure Mode place this vulnerability in a contained environment where exploitation leads to no durable compromise.
...
HeartSuite Secure Mode enforces an SPF allowlist in `hs_sandbox_caching.c` that applies equally to root. Reaching the io_uring provided-buffer path requires an approved process to submit SQEs using `IOSQE_BUFFER_SELECT`. An attacker cannot execute a new exploit binary — it has no allowlist entry and the kernel refuses to run it. Together, Lockdown and Secure Mode place this vulnerability in a contained environment where exploitation leads to no durable compromise.
...
HeartSuite Secure Mode enforces an SPF allowlist in `hs_sandbox_caching.c` that applies equally to root. Reaching this path requires an approved process to manipulate extended attributes on an ext4 filesystem with a crafted xattr block. An attacker cannot execute a new exploit binary — it has no allowlist entry and the kernel refuses to run it. Together, Lockdown and Secure Mode place this vulnerability in a contained environment where exploitation leads to no durable compromise.
...
`CONFIG_IPV6=y` is compiled in and 5.19.6 falls within the affected range. Any process that can open a `NETLINK_ROUTE` socket can trigger the leak; no elevated privilege is required beyond the netlink socket permission.

**HeartSuite Lockdown** (`sys_hs_lockdown_hs()`, `kernel/ioctl.c:561–569`; `kernel/namespace.c:4218, 4300, 4453`) does not reduce the integrity score — the base vector already carries I:N, so there is no integrity component to constrain. Lockdown closes the post-exploitation window instead: an attacker who chains the infoleak into a larger exploit cannot mount filesystems, set persistent flags via `FS_IOC_SETFLAGS`, or survive a reboot.
...
The combination keeps a successful attacker contained: the infoleak byte is present, but the environment prevents translating leaked kernel memory into persistent compromise or lateral movement.
...
**Base Score**: 7.8 HIGH (AV:L/AC:L/PR:L/UI:N/S:U/C:H/I:H/A:H)
**Environmental Score**: 0.0 — no LAM-using process in HS allowlist

Linear Address Masking (LAM) has a weakness related to transient execution as described in the SLAM paper[1].

`CONFIG_X86_64=y` is compiled in and 5.19.6 falls within the affected range. The vulnerable code path requires a process to invoke Linear Address Masking (LAM) via `ARCH_SET_GS`/`ARCH_SET_FS` with a tagged pointer. No approved process in the HS allowlist uses LAM or invokes this kernel interface; on a HeartSuite Core Secure system, the SPF allowlist blocks execution of any unapproved binary at the kernel gate before any interface interaction is possible. After gaining root through any avenue, Lockdown closes every follow-on path: root cannot modify the allowlist, install persistent backdoors, or survive a reboot.
