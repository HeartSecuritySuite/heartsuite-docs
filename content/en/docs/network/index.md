...
**Overview**: For internet access, allowlist specific IPs—HeartSuite blocks everything by default to prevent exploits.
...
The following permission error message concerning network access appears in the log file. Use the Dashboard to access the guided network review queue with suggested next steps.

```text
[Setup Notice - Add to Network Allowlist?] Network Connection Attempt Logged by /usr/bin/wget; IP: 45.60.22.168
```

You can add this IP address to the allowlist entry for wget as follows (or use the Dashboard's guided review queue for the interaction pattern):
