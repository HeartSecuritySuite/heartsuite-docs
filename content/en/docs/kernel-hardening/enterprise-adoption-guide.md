...
|---|---|
| Need kernel-level per-program execution + file + network control that survives compromised root, and can accommodate the HeartSuite kernel | Root Lock by HeartSuite with HeartSuite kernel |
...
| Want both layers | Root Lock by HeartSuite (execution/network) + HJFS (file isolation and versioning) on the same host where the HeartSuite kernel is acceptable |
