...
- `the sandbox-enforcement checklist` — Architectural Invariants section: "The build disabled 6 kernel CONFIG options that each provide a mechanism to bypass VFS-level enforcement."
- `the deployment-hardening checklist` item 1 — per-option rationale for each disable.
