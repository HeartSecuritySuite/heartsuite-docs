...
After narrowing kmod's file access permissions, reboot and confirm the system starts normally with no kmod access denials in the review queues. Then activate Lockdown from the Lockdown button (`[l]`). If kmod still has directory-level access at that point, the Dashboard shows it in the review (as advisory) before confirmation. You can opt back in via `[u]` undo (priority after install) or adjust before typing `YES` (case-sensitive). See [Mode Switching and Lockdown](../../mode-switching/) for prep actions.
