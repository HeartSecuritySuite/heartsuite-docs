...
> identified that security had become an incoherent patchwork of disconnected tools, each addressing a narrow concern with no unified design. This diagnosis—and the conviction that a single principled architecture could replace the patchwork—became the founding motivation behind HeartSuite. The idea was developed with a research collaborator. No code yet: only the problem statement and the conviction that a coherent solution was possible.
...
> *Zero Day Secure: Why Modern Operating Systems Can't Stop Malware and How to Fix Them*, published by Simon & Schuster (ISBN 9781968865078). The book presents the architectural argument that HeartSuite implements in code: that operating systems must be redesigned to prevent malware execution at the kernel level, not detect it after the fact. It is the written form of the insight that motivated nine years of research and development—from the first frustration in 2016 to a running kernel in production.
