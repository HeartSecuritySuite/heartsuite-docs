...
- **hs-cache-size** -- used to change the maximum number of whitelist entries that may be cached simultaneously; please view program help for details (adjust for performance)
- **hs-whitelist-manager** – manages access permissions that allow programs to run, as well as which directories and remote computers they may access; please view program help for details (core tool for permissions)
...
- **hs-os-boot-setup.py** – scans the HeartSuite log and the kernel log for permission error messages and uses them to construct appropriate whitelist entries
- **batch_record_add.py** -- adds programs listed in a file to whitelist entries, as well as basic directory access
- **batch_record_add_root.py** -- adds programs listed in a file to whitelist entries, as well as read access to all files; USE THIS SCRIPT WITH CAUTION, IF AT ALL
...
- **init_base_records.sh** -- used by the installation script to add the Linux Standard Base (LSB) programs to whitelist entries, as well as certain essential programs; this script is used only once-- during Part 1 of installation
- **HS_lockdown.sh** -- makes files and directories immutable using chattr, then executes the hs-lockdown program; EDIT THIS SCRIPT TO MEET YOUR NEEDS
