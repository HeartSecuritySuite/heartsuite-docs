...
**File Backup** keeps versioned copies of signing keys and build output. Under Lockdown, the kernel blocks any program (including root) from reaching those copies — so even if an approved tool is compromised, the previous versions remain intact and restorable from the Dashboard's Backup.
