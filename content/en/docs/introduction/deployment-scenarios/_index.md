...
A supply chain attacker who compromises the build pipeline itself — using its own credentials and tooling — does not introduce a new program; in the May 2026 TanStack incident, 84 malicious package versions across 42 packages were published in six minutes using valid pipeline credentials. The execution gate fires but does not block: the pipeline already has a valid allowlist entry. The attack is contained, not prevented at execution. The network allowlist still blocks: a compromised build tool cannot reach destinations outside its approved list, regardless of credential validity. Network allowlist entries are literal IP addresses — if an attacker's exfiltration endpoint shares an IP with an already-approved destination, the gate passes it. File Backup keeps versioned copies of signing keys that no allowlisted program, including the pipeline itself, can overwrite while Lockdown is active — even as root. Backup protection prevents destruction or replacement of the key; a compromised program with legitimate read access to the live key can still use it to sign malicious artifacts.
