...
- **Hypervisor hosts running virtual machines** — HeartSuite Core Secure protects workloads running *inside* a kernel; a hypervisor host runs the inverse model, granting trusted access to guest workloads it does not control. The HS kernel is built for the first case, not the second. KVM host mode — HeartSuite Core Secure acting as the hypervisor — is not a supported configuration; the kernel features KVM requires have been compiled out. HeartSuite Core Secure runs as a VM guest on KVM, cloud hypervisors, and other platforms, and provides kernel-level enforcement inside the guest — which is the deployment shape this product was designed for.
