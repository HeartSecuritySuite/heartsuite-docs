...
- **Shared-kernel container guests (OpenVZ, LXC, Docker/Podman guests on a provider kernel, systemd-nspawn)** — HeartSuite must boot its own kernel by design. Full VMs under KVM or cloud hypervisors are supported; Root Lock by HeartSuite runs as a guest kernel inside them.
