...
For CISO and procurement evaluation of the HeartSuite kernel itself in enterprise fleets (including operational models, Secure Boot roadmap, and alternatives), see the [Kernel Hardening → Enterprise Adoption Guide](../../kernel-hardening/enterprise-adoption-guide/).
...
Compare this to the products in the first table: in most of them, root is sufficient to disable enforcement by killing or unloading a control plane. Root Lock by HeartSuite is deliberately not in that category.
