...
type: docs
aliases:
  - /docs/security/compiled-in-cves/
---
...
**The attacker cannot turn this UAF into anything that runs new code.** Even if a follow-on memory-corruption bug is chained in to escalate to root, Lockdown's allowlist refuses every non-allowlisted program at `execve`, including in the worst case where the attacker has cleared Lockdown. No persistence, no backdoors, no cross-reboot survival. ([How](/rootlock/security/#how-to-read-the-backstop-sections).)
