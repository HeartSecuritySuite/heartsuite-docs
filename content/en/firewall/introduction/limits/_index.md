...
**What it does not cover.** Content on an allowed port is outside a stateful packet filter. A listener that binds a port the image already left open (including baseline ports open to any source) is still that approved path. Packets to ports off the allowlist still fail.

A WAF, application hardening, and [Root Lock by HeartSuite](../../../docs/) (what that process may execute, read, write, and call outbound) address the blast radius inside the approved service. This is a deliberate division of layers.
...
**What Root Lock Firewall enforces.** The host path is still subject to the sealed host filter. That is independent of per-program outbound policy.

**What it does not cover.** Which *program* may reach which *literal IP* stays [Root Lock](../../../docs/network/). The sealed host filter on this image still holds.
...
**What it does not cover.** v1 stays host-shaped. FORWARD and NAT stay later. The sealed allowlist of *this* image still holds. See [Deployment scenarios](../../deployment-scenarios/).
...
**What it does not cover.** App-ID, TLS interception, and URL clouds stay with the specialist tool. The sealed host allowlist on this image still holds.
...
**What it does not cover.** Physical presence, a cloud serial console, or control of the hypervisor under a virtual appliance returns the box to whoever holds that path. Restrict console access in the hypervisor or cloud IAM. The sealed allowlist still holds against remote rewrite.
...
**What it does not cover.** Seal is immutability of the set you chose. Reduce through Maintenance. The sealed set still holds until you unseal it.
